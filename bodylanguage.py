...
shaky_hand_movement_threshold = 0.05  # Example threshold for larger vibrations in hand movement

# Buffer to track previous hand movements for shaking detection
...
def classify_shrugging(landmarks):
    ...
    if shoulder_height < ear_height:
        return "Shrugging Shoulders - Nervous"
    elif abs(left_shoulder.y - left_ear.y) > 0.05 or abs(right_shoulder.y - right_ear.y) > 0.05:
        return "Shoulders Tense - Discomfort"
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        hand_movement_message = classify_hand_movements(landmarks, prev_landmarks, hand_movement_buffer)
